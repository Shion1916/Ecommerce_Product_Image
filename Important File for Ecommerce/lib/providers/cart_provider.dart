...
class CartProvider with ChangeNotifier {
  ...
  Future<void> _fetchUserEmailFromFirestore() async {
    ...
    try {
      ...
      if (doc.exists && doc.data()!['email'] != null) {
        ...
        print('User email fetched from Firestore: $_userEmail');
      } else {
        _userEmail = 'unknown@example.com';
        print('No email found for user $_userId');
      }
    } catch (e) {
  ...
  void initializeAuthListener() {
    ...
    _authSubscription = _auth.authStateChanges().listen((User? user) async {
      if (user == null) {
        ...
        await _fetchUserEmailFromFirestore();
  ...
  Future<void> placeOrder() async {
    ...
    try {
      ...
      await _firestore.collection('orders').add({
        'userId': _userId,
        'userEmail': _userEmail ?? 'unknown@example.com',
        'items': cartData,
        'subtotal': subtotal,
        'vat': vat,
      ...
      print('Order placed successfully for $_userEmail');
    } catch (e) {
